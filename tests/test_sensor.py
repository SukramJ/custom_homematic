...
async def test_sensor_to_trans(factory: helper.Factory) -> None:
    ...
    await control.central.data_point_event(const.INTERFACE_ID, "VCU7837366:1", "Abs_Luftfeuchte", 1)
    await hass.async_block_till_done()
    assert hass.states.get(entity_id).state == "1.0"

    await control.central.data_point_event(const.INTERFACE_ID, "VCU7837366:1", "Abs_Luftfeuchte", 0)
    await hass.async_block_till_done()
    assert hass.states.get(entity_id).state == "0.0"
